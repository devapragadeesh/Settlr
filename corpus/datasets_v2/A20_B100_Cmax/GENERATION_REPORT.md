...
spine. Closest to the frozen primary set: small pool, full attestation, the documented reading (B). Any sound resolver must score near-perfectly here. v2: one batch's settlement_id names rows that are not its true composition, and the arithmetic still closes.

| axis | target | achieved |
|---|---|---|
| A pool size | 20 | mean 18.17, sizes [4, 23, 20, 9, 20, 21, 21, 24, 18, 29, 15, 14] |
| B attestation coverage | 1 | 12/12 |
| C selection rule | max_under_cap | max_under_cap (phi=9/10) |

seed `20260911`, committed before generation.
...
- truth present in the closure register: {'True': 11, 'yes': 1}
...
- posting lag histogram (days): {0: 7, 1: 4, 5: 1}
