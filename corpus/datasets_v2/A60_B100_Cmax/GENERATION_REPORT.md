...
above max_pool, so the simulator degrades to FIFO and says so. Closure is capped and the register says THAT too. v2: one batch's settlement_id names rows that are not its true composition, and the arithmetic still closes.

| axis | target | achieved |
|---|---|---|
| A pool size | 60 | mean 53.17, sizes [7, 61, 59, 60, 58, 38, 66, 61, 55, 55, 64, 54] |
| B attestation coverage | 1 | 12/12 |
| C selection rule | max_under_cap | max_under_cap (phi=9/10) |

seed `20260914`, committed before generation.
...
- ERP invoices: 710; GSTR-2B lines: 23
...
- reference gaps: min 4, max 39 (a dense sequence would be a counter minted for this file)
