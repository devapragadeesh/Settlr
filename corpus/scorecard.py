...
#: The D15 verdict, measured in. Held here
...
#:. Held here as data for the same reason D15 is: this
...
#: `ingest/ingestion_results.json`, written by `ingest/ingestion_report.py`
#:. Unlike D15/SCALE above, this is NOT a held
#: constant -- the underlying sweep costs seconds, not minutes, so the
#: scorecard reads the file fresh on every render rather than pinning a
#: point-in-time number.
INGESTION_RESULTS_PATH = ROOT / "ingest" / "ingestion_results.json"
...
def main() -> int:
    ...
    rows = [
        ...
        ("both FAILING datasets fail on", "**a premise**",
         ...
         "than the resolver's. The gate is **not** loosened"),
        ("", "", ""),
        ("**The exception queue**", "", ""),
        ...
        ("`OpenBreak` rows — classified, aged, **assert nothing**",
         ...
         "never summed with the row above"),
        ...
        (f"at {SCALE['rows_at_largest']:,} rows",
         ...
         "`scale/RESOLVER_SCALE_REPORT.md`"),
    ]

    if INGESTION_RESULTS_PATH.exists():
        ingestion = json.loads(INGESTION_RESULTS_PATH.read_text())
        rows.append(("", "", ""))
        rows.append(("**Ingestion adapters — round-trip against every dataset**", "", ""))
        for name, counts in ingestion.items():
            rows.append((
                f"`{name}`",
                f"**{counts['ok']}/{counts['total']}**",
                "fixtures generated from this repo's own data, not a real "
                "bank export -- `ingest/INGESTION_REPORT.md`"))
