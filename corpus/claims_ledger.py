...
UNTRACEABLE = [
    ...
    ("closure count over the DERIVED pool at the 18 reconstructible instances",
     ...
     "actually see, and at `A20_Bnone_Cmax` line 1 it proved so exhaustively "
     "(178 subsets, enumeration complete). It stays in THIS table because no "
     "command regenerates it: `corpus/scorecard.py` carries the figure as a "
     "held constant sourced from that document. Not reproducible is not the "
     "same as not measured, and this row conflated them"),
]
