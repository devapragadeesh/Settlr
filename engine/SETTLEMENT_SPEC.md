...
| **Settlement reversals / failed payouts** | the most common real recon exception at any aggregator, and **deferred to held-out validation (Phase 4)**. Modelling one requires a bank-side debit reversing a prior credit and a re-settlement under a new UTR — a second bank-statement shape THIS dataset does not have. Rather than regenerate the frozen set, the shape is introduced only in `holdout/`, as a class the engine has provably never seen; see `holdout/HOLDOUT_SPEC.md`. Named as a gap rather than quietly omitted |
